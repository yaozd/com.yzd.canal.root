...
import com.yzd.canal.example.cacheSetting.RedisCacheConfig;
import com.yzd.canal.example.model.TableInfo;
import com.yzd.canal.example.model.TableWithKey;
...
public class T5 {
    ...
    private static void printEntry(@NotNull List<CanalEntry.Entry> entrys) {
        ...
        for (RecordInfo recordInfo:recordInfoList){
            List<TableWithKey> tableWithKeyList= TableInfo.getInstance().getKeyByTable(recordInfo.getTableName());
            ShardedRedisUtil redisUtil = ShardedRedisUtil.getInstance();
            for (TableWithKey tableWithKey:tableWithKeyList){
                String timestampKeyName= RedisCacheConfig.getKeyFullNameForTimestamp(tableWithKey.getKey());
                //timestampKeyValue
